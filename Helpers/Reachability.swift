...
extension Reachability {
    ...
    /// Flags that indicate the reachability of a network node name or address, including whether a connection is required,
    ...
    /// The specified node name or address can be reached using the current network configuration,
    /// but a connection must first be established. Any traffic directed to the specified name or address will initiate the connection.
    ...
    /// The specified node name or address can be reached using the current network configuration,
    /// but a connection must first be established. If this flag is set
    /// The specified node name or address can be reached via a transient connection, such as PPP.
    var isRequiredAndTransient: Bool {
...
struct Network {
    ...
    static func checkConnection() -> Bool {
        ...
        guard let status = reachability?.status else { return false}
        switch status {
        case .unreachable:
            return false
        default :
